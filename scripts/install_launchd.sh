#!/bin/bash
# Installs two macOS launchd agents: full auto-sync (Gmail, Calendar,
# Docs, local files) every 10 minutes, and a nightly digest run (which
# itself checks DIGEST_DAYS in .env to decide whether today is a
# scheduled day). Safe to re-run - reloads cleanly if already installed.
set -euo pipefail
...
chmod +x "$PROJECT_DIR/scripts/sync_all.sh" "$PROJECT_DIR/scripts/nightly_digest.sh"
...
echo ""
echo "Logs: $PROJECT_DIR/logs/launchd-autosync.log and $PROJECT_DIR/logs/launchd-digest.log"
